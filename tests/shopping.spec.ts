...
test("Validate that user able to place the order", async({page}) => {
    ...
    await test.step("Add the product to the cart", async() => {
        await dashboardPage.getProduct('ZARA COAT 3').addToCartButton.click();
    ...
    await test.step("Validate product details in cart and do checkout", async() => {
        await expect(cartPage.getProduct('ZARA COAT 3').productName).toHaveText("ZARA COAT 3");
...
test("Validate that user able to place the order for iPhone", async({page}) => {
    ...
    await test.step("Add the iPhone to the cart", async() => {
        await dashboardPage.getProduct('iphone').addToCartButton.click();
        await expect(dashboardPage.cartProductsCount).toHaveText('1');
        await dashboardPage.navigateToCartPage();
        await expect(cartPage.base).toBeVisible();
    });
    ...
    await test.step("Validate iPhone in cart and do checkout", async() => {
        await expect(cartPage.getProduct('iphone').productName).toHaveText("iphone");
        await cartPage.checkoutButton.click();
        await expect(checkoutPage.emailInput).toBeVisible();
    });
